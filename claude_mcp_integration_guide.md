...
| 항목 | 이전 | 현재 |
|------|------|------|
| **설정 파일** | settings.json만 | **`.mcp.json` (팀 공유), `~/.claude.json` (개인)** |
| **전송 방식** | stdio만 | **stdio, HTTP (권장), SSE (deprecated)** |
| **인증** | 환경변수 수동 | **OAuth 2.0 자동 인증** |
| **도구 로딩** | 전부 사전 로딩 | **MCP Tool Search (동적 로딩)** |
| **리소스** | 미지원 | **`@server:protocol://path` 멘션** |
| **프롬프트** | 미지원 | **`/mcp__서버__프롬프트` 명령어** |
| **관리 정책** | 미지원 | **Managed MCP (allowlist/denylist)** |
| **환경변수** | `${VAR}` | **`${VAR:-기본값}` 구문 추가** |
...
2. [Claude Code에서 MCP 설정](#2-claude-code에서-mcp-설정)
   ...
   - 2.2 설정 파일 구조 및 스코프
   - 2.3 전송 방식 (Transport Types)
   - 2.4 OAuth 2.0 인증
   - 2.5 MCP 서버 실행 패턴
   - 2.6 CLI로 MCP 서버 관리
   - 2.7 환경변수 확장
   - 2.8 디버깅
3. [MCP 신규 기능 (v2.1.72)](#3-mcp-신규-기능-v2172)
   - 3.1 MCP Tool Search (동적 도구 로딩)
   - 3.2 MCP Resources (@멘션)
   - 3.3 MCP Prompts as Commands
   - 3.4 Managed MCP (관리 정책)
   - 3.5 Claude Code as MCP Server
4. [Skill + MCP 통합 패턴](#4-skill--mcp-통합-패턴)
   - 4.1 Skill에서 MCP 도구 호출
   - 4.2 여러 MCP 서버 조합
   - 4.3 실전 예시: 기술 경쟁분석 Skill
5. [Agent + MCP 통합 패턴](#5-agent--mcp-통합-패턴)
   - 5.1 전문화된 Agent에 MCP 도구 할당
   - 5.2 Agent별 MCP 서버 분리 전략
   - 5.3 실전 예시: 코드 리뷰 Agent
6. [멀티 에이전트 오케스트레이션 + MCP](#6-멀티-에이전트-오케스트레이션--mcp)
   - 6.1 Agent-MCP 프레임워크 패턴
   - 6.2 Handoff 패턴
   - 6.3 Reflection 패턴
   - 6.4 병렬 실행 패턴
7. [인기 MCP 서버 활용 가이드](#7-인기-mcp-서버-활용-가이드)
   - 7.1 GitHub MCP
   - 7.2 Database MCP
   - 7.3 AWS MCP
   - 7.4 Zapier MCP
   - 7.5 Playwright MCP
   - 7.6 Context7
8. [보안 Best Practices](#8-보안-best-practices)
   - 8.1 자격 증명 관리
   - 8.2 환경 분리
   - 8.3 읽기 전용 우선 원칙
   - 8.4 SSO 및 RBAC 통합
   - 8.5 2025년 보안 이슈 및 대응
9. [Hook + MCP 연계](#9-hook--mcp-연계)
   - 9.1 MCP 도구 호출 전/후 Hook
   - 9.2 MCP 결과 검증 Hook
   - 9.3 자격 증명 자동 주입 패턴
   - 9.4 Prompt-Based MCP Validation
10. [실전 워크플로우 예시](#10-실전-워크플로우-예시)
    - 10.1 기본: GitHub PR 자동 리뷰
    - 10.2 중급: 멀티소스 경쟁분석 파이프라인
    - 10.3 고급: 풀스택 배포 오케스트레이션
...
**v2.1.72에서 3개 스코프로 확장:**

| 스코프 | 파일 | 범위 | Git 공유 |
|--------|------|------|---------|
| **Project** | `.mcp.json` (프로젝트 루트) | 팀 전체 | ✅ |
| **Local** | `~/.claude.json` (프로젝트별) | 개인 | ❌ |
| **User** | `~/.claude.json` (크로스 프로젝트) | 모든 프로젝트 | ❌ |
| **Managed** | `managed-mcp.json` (시스템) | 조직 전체 | N/A |

**우선순위**: Managed > Local > Project > User

**`.mcp.json` (팀 공유, 프로젝트 루트에 커밋):**
```json
{
  "mcpServers": {
    "github": {
...
| 항목 | 설명 | 전송 방식 |
|------|------|----------|
| `type` | `stdio`, `http`, `sse` | 모두 |
| `command` | 실행 명령어 | stdio |
| `args` | 명령어 인자 | stdio |
| `env` | 환경 변수 | stdio |
| `url` | 서버 URL | http, sse |
| `headers` | HTTP 헤더 | http, sse |
| `oauth` | OAuth 2.0 설정 | http |

**권장 패턴:**
```
.mcp.json (Project): 팀 공유 서버 (Git 커밋)
~/.claude.json (Local): 개인 토큰이 필요한 서버
~/.claude.json (User): 모든 프로젝트 공통 서버
```
...
**특징:**
- 토큰 자동 갱신
- Client secret은 키체인/자격 증명 파일에 안전 저장 (설정 파일에 미포함)
- CI 환경: `MCP_CLIENT_SECRET` 환경변수 사용

### 2.5 MCP 서버 실행 패턴

핵심: **"그 도구를 어떻게 실행하느냐"가 다 다름**
...
**비유: 음식 주문 방식이 가게마다 다른 것과 같음**

| 방식 | 비유 | 설명 |
|------|------|------|
| `codex mcp` | 맥도날드 키오스크 | 자체 주문 시스템 |
| `npx` 패키지 | 배달앱 | 앱으로 주문 |
| `node server.js` | 집밥 | 직접 요리 |

→ "MCP 서버를 실행한다"는 목적은 같지만 방법이 다름

**실행 방식 정리:**

| 도구 | 실행 명령 | 이유 |
|------|----------|------|
| Codex | `codex mcp` | Codex가 MCP 모드를 내장함 |
| GitHub | `npx -y @modelcontextprotocol/server-github` | npm 패키지로 배포됨 |
| 내 서버 | `node server.js` | 직접 만듦 |

> **핵심**: `이름 mcp` 형식이 아니라, **각 도구가 제공하는 방식을 그대로 쓰면 됩니다.**
...
```bash
claude mcp add --scope local <name> ...     # ~/.claude.json (기본)
claude mcp add --scope project <name> ...   # .mcp.json (승인 필요)
claude mcp add --scope user <name> ...      # ~/.claude.json (크로스 프로젝트)
```
...
**인터랙티브 메뉴:**
```
/mcp    # Claude Code 내에서 MCP 서버 관리
```

### 2.7 환경변수 확장

`.mcp.json`에서 환경변수를 참조할 수 있습니다.

**지원 구문:**
- `${VAR}` — 환경변수 확장
- `${VAR:-default}` — 기본값 포함 (변수 미설정 시 default 사용)
...
MCP 도구 정의가 컨텍스트 윈도우의 **10% 이상**을 차지하면 자동으로 **동적 로딩 모드**가 활성화됩니다.

**동작:**
1. 도구 정의를 사전 로딩하는 대신 지연(deferred)
2. Claude가 필요한 도구를 온디맨드로 검색
3. 필요한 도구만 컨텍스트에 로딩
...
MCP 서버의 리소스를 프롬프트에서 직접 참조할 수 있습니다.
...
**사용 예시:**
```
@github:issue://123 을 분석하고 수정 방안을 제시해줘
@postgres:schema://users 테이블과 @docs:file://database/user-model 을 비교해줘
```

**탐색:**
- `@` 입력 시 사용 가능한 리소스 자동완성
- 퍼지 검색 지원
- 참조 시 자동 페치

### 3.3 MCP Prompts as Commands

MCP 서버의 프롬프트를 슬래시 명령어로 실행할 수 있습니다.
...
- `/` 메뉴에서 `mcp__`로 시작하는 명령어로 표시
- 인수는 프롬프트에 정의된 파라미터에 따라 파싱

### 3.4 Managed MCP (관리 정책)

조직 관리자가 MCP 서버를 중앙 관리할 수 있습니다.

**방법 1: 완전 통제 (managed-mcp.json)**

시스템 경로에 배포 (관리자 권한 필요):
...
```json
{
  "allowedMcpServers": [
    ...
    { "serverCommand": ["npx", "-y", "@mcp/server-filesystem"] }
  ],
  "deniedMcpServers": [
...
**규칙:**
- 빈 allowlist `[]` = 완전 잠금 (모든 서버 차단)
- 미정의 allowlist = 제한 없음 (기본)
- denylist가 allowlist보다 우선

### 3.5 Claude Code as MCP Server

Claude Code 자체를 MCP 서버로 실행할 수 있습니다:

```bash
claude mcp serve
```

다른 MCP 클라이언트에서 Claude Code의 도구를 사용할 수 있습니다.

### 3.6 추가 설정

```bash
# MCP 출력 제한 (10,000 토큰 초과 시 경고, 기본 최대 25,000 토큰)
...
**환경별 설정 파일 (커스텀 패턴, 수동 전환 필요):**
```bash
.claude/
├── settings.json              # 기본 (개발)
├── settings.staging.json      # 스테이징 (수동 복사/심링크로 전환)
└── settings.production.json   # 프로덕션 (수동 복사/심링크로 전환)
```
> **참고:** Claude Code는 환경별 설정 파일을 자동 인식하지 않습니다.
> 배포 스크립트에서 해당 환경 파일을 `settings.json`으로 복사하거나 심링크하세요.
...
- [Claude Code MCP 공식 문서](https://code.claude.com/docs/en/mcp.md) - 최신 MCP 설정 가이드
